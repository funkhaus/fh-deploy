...
module.exports.default = async function(config, queue) {
    ...
    if (
        ...
        !config.settings.password.length
    ) {
        config.settings = config.settings || {}
        config.settings.password = prompt('Password: ', { echo: '*' })
    }

    const connection = await sftp
    ...
    if (connection instanceof Error) {
        ...
        console.log('Login failed. No action taken.'.red.bold)
        await sftp.end()
        return
    }

    // make sure the target path is valid
    const targetPathCheck = await sftp
        .list(config.target)
        .catch(err => new Error(err))
    if (targetPathCheck instanceof Error) {
        console.log(targetPathCheck)
        console.log('Target directory is invalid. No action taken.'.red.bold)
        await sftp.end()
        return
    }
    ...
    try {
        for (let item of queue) {
            ...
            if (item.includes('/')) {
                ...
                await sftp
                    .mkdir(path.resolve(config.target, dirPath) + '/', true)
                    .catch(err => {
                        throw new Error(err)
                    })
            }
            ...
            if (config.lazy) {
                ...
                const lastModifiedLocalMs = _get(
                    fs.statSync(path.resolve('./', item)),
                    'mtimeMs',
                    1
                )
                ...
                const remoteStats = await sftpStat(
                    sftp,
                    path.resolve(config.target, item)
                ).catch(() => {
        ...
        console.log(err)
        console.log(
            'Error during uploading. Not all files may be uploaded.'.yellow
        )
        hasError = true
    }
